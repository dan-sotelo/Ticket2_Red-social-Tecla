...
module.exports = async(app) =>{
    ...
    app.post('/teclers/perfil/hobbies', middUsuarios.validarToken, middUsuarios.validarCredenciales, middUsuarios.datosHobbies, async(req, res) =>{
        let usuario = req.params.usuario;
        let infoHobby = req.body;
        try{
            let hobby = await controladorHobbies.registrarHobbies(usuario.id_usuario, infoHobby);
            res.status(200).json({message: 'Registro exitoso', hobby})
        } catch(error){
    ...
    app.patch('/teclers/perfil/hobbies/:idHobbyUsuario', middUsuarios.validarToken, middUsuarios.validarCredenciales, middUsuarios.datosHobbies, async(req, res) =>{
        let usuario = req.params.usuario;
    ...
    app.delete('/teclers/perfil/hobbies/:idHobbyUsuario', middUsuarios.validarToken, middUsuarios.validarCredenciales, async(req, res) =>{
        let usuario = req.params.usuario;
        let idHobbyUsuario = req.params.idHobbyUsuario;
        try{
    ...
    app.get('/teclers/perfil/hobbies', middUsuarios.validarToken, middUsuarios.validarCredenciales, async(req, res) =>{
        let usuario = req.params.usuario;
        try{
            let hobbies = await controladorHobbies.listarHobbies(usuario.id_usuario);
            res.status(200).json({message: 'Consulta exitosa', hobbies})
        } catch(error){
            console.log(error.message);
            res.status(500).json({message: error.message});
        }
